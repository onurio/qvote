...
describe("handleOpenVoteModal", () => {
  ...
  it("returns error when vote not found", async () => {
    ...
    const actionWithNonExistentVote = createMockAction(`vote_${nonExistentVoteId}`);
    const mockPayload = createMockPayload(mockUserId);
  ...
  it("returns error when workspace token not found", async () => {
    ...
    assertEquals(response.status, 200);
    assertStringIncludes(response.body.text || "", "Workspace not found or authentication error");
  });
